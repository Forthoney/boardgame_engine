...
EXIT_INSTRUCTIONS = "Try a sample input or input 'back' to leave the tutorial. \
Type in 'exit' anytime to exit the game fully"
...
class Board
  ...
  def display(show_row: false, show_col: true)
    if show_row
...
class Boardgame
  ...
  def tutorial
    puts @instructions + EXIT_INSTRUCTIONS
    input = gets.chomp
    until input == 'back'
      exit if input == 'exit'
